  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <section id="catalog" className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-8 w-full flex-1">
        ...
        {/* Pricing Calculator & Checkout Panel (Desktop) */}
      ...
      <AnimatePresence>
        {isMobilePanelOpen && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 lg:hidden">
            {/* Background overlay */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            <motion.div
              ...
              className="bg-white rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl relative z-10 w-full max-w-lg max-h-[85vh] overflow-y-auto"
            >
              <button
                onClick={() => setIsMobilePanelOpen(false)}
                className="absolute top-4 right-4 text-slate-400 hover:text-slate-700 h-8 w-8 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100 active:scale-95 transition"
              >
                ✕
              </button>
              {renderCheckoutPanel()}
      ...
      {activeRoomType && !isMobilePanelOpen && bookingStep !== 'success' && (
        <div className="fixed bottom-6 left-0 right-0 px-6 z-40 lg:hidden flex justify-center">
          <button
            onClick={() => setIsMobilePanelOpen(true)}
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs px-6 py-3.5 rounded-full shadow-lg flex items-center space-x-2 active:scale-95 transition-all border border-indigo-500/30"
          >
